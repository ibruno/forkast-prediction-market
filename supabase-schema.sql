...
-- Wallet Indexes
CREATE INDEX idx_wallets_signer ON wallets(signer);
...
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Service role full access' AND tablename = 'objects' AND schemaname = 'storage') THEN
